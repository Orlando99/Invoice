...
*/
			...
			createEstimateReceipt : function(estimateId) {
				...
				return query.get(estimateId)
					...
					data.estimateObj.set("emailReceipt", data.emailHtml);
			...
			sendEstimateReceiptToEmail : function(estimate, email) {
				...
				//var link = est.entity.estimateReceipt.url();
				var link = est.entity.emailReceipt.url();
				return $.ajax({
					type: "GET",
					url: 'proxy.php',
					dataType: "html",
					...
					fr.setAttribute("id", "myFrameEstimate");
					...
					fr.onload = function() {
						...
						return Parse.Cloud.run("sendMailgunHtml", {
							...
							html : '<html>' + $('#myFrameEstimate').contents().find('html').html() + '</html>'
						}).then(function(msg) {
							console.log(msg);
							return estimate;
						});
